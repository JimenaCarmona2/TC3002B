...
class Quadruple:
    ...
    def __repr__(self): # representacion en cadena para imprimir
        def f(v): return '_' if v is None else str(v) # '_' es campo vacío (=, 2, _, a)
...
class QuadrupleGenerator:
    ...
    def new_temporal(self):
        self.temp_cnt += 1 # t(n+1)
        return f"t{self.temp_cnt}"

    def add_quad(self, op, left, right, res):
        self.quadruple_queue.append(Quadruple(op, left, right, res)) # al final de quadruple_queue
    ...
    def gen_quad_relational(self, op): # recibe operador porque los relacionales no se apilan en operator_stack
        right = self.operand_stack.pop()
        left = self.operand_stack.pop()
        right_type = self.type_stack.pop()
        left_type = self.type_stack.pop()
    ...
    def gotof(self):
        ...
        self.jump_stack.append(self.quadruple_count() - 1) # el índice para backpatch
    ...
    def backpatch(self, idx):
        self.quadruple_queue[idx].res = self.quadruple_count() # llena el destino de gotof/goto con el índice actual
    ...
    def print_quad(self):
        ...
        for i, q in enumerate(self.quadruple_queue):
            def f(v): return '_' if v is None else str(v)
            print(f"  [{i}]  {f(q.op):<8} {f(q.left):<12} {f(q.right):<12} {f(q.res):<10}")
        print("=" * 54)